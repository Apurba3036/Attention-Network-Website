...
const CheckOut = () => {
    const {_id, title, image, price} = useLoaderData();
    ...
    const handlebookservice = async (event) => {
        ...
        try {
            const allBookingsResponse = await fetch('http://localhost:5000/allBookings');
            ...
            const isDateBooked = allBookings.some(b => b.date === date) || 
              allPayments.some(p => 
              (Array.isArray(p.service_date) && p.service_date.includes(date)) || 
              p.service_date === date
          );
            ...
            if (isDateBooked) {
                Swal.fire({
                    ...
                    text: "The selected date is already booked. Please choose another date.",
                    icon: "error"
                });
            } else {
                Swal.fire({
            ...
            console.error('Error fetching bookings or payments:', error);
        }
    };
